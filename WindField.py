...
class WindField:
    ...
    def simulate_multi_step_gp(self, max_size=50, num_steps=5, show=False, save=None):
        ...
        # Simulate the field 
        t = 0
        predicted_wind_force_x = None
        predicted_wind_force_y = None
        ...
        for target_p, target_v in self.__trajectory:
            ...
            # Total force
            force = wind_force + control_force
            point = torch.FloatTensor([[self.__system.p[0],self.__system.p[1]]])
            idx = (t-max_size)%num_steps
            if t>=max_size and idx==0:
                # Make num_steps ahead predictions
                p = torch.FloatTensor([tr[:,t:t+num_steps]]).reshape((num_steps,2))
                predicted_wind_force_x = self.__gp_predictor_x(p)
                predicted_wind_force_y = self.__gp_predictor_y(p)
            if predicted_wind_force_x is not None:
                force -= np.array([(predicted_wind_force_x.mean)[idx].item(),(predicted_wind_force_y.mean)[idx].item()],dtype=float)
                # Collect Data For Plot
                x_pred.append((predicted_wind_force_x.mean)[idx].item())
                y_pred.append((predicted_wind_force_y.mean)[idx].item())
                lower, upper = predicted_wind_force_x.confidence_region()
                x_lower.append(lower[idx].item())
                x_upper.append(upper[idx].item())
                lower, upper = predicted_wind_force_y.confidence_region()
                y_lower.append(lower[idx].item())
                y_upper.append(upper[idx].item())
            ...
            # Update GP Model
            if t==0:
                self.__gp_predictor_x.set_train_data(point,torch.FloatTensor([wind_force[0]]),strict=False)
                self.__gp_predictor_y.set_train_data(point,torch.FloatTensor([wind_force[1]]),strict=False)
            elif t>=max_size:
                # Changing the training data of the model each iteration has a cost of O(M^2),
                # where M is the number of points used in the model. 
                # Computing the posterior has a cost of O(M^3). The complexity of recomputing the Gram
                ...
                gp_labels_x = self.__gp_predictor_x.train_targets
                gp_labels_y = self.__gp_predictor_y.train_targets
                q = p[idx].clone().reshape(1,2)
                self.__gp_predictor_x.set_train_data(torch.cat([gp_data[1:,],q],dim=0),torch.cat([gp_labels_x[1:],torch.FloatTensor([wind_force[0]])]),strict=False)
                self.__gp_predictor_y.set_train_data(torch.cat([gp_data[1:,],q],dim=0),torch.cat([gp_labels_y[1:],torch.FloatTensor([wind_force[1]])]),strict=False)
            else:
                gp_data = self.__gp_predictor_x.train_inputs[0]
                gp_labels_x = self.__gp_predictor_x.train_targets
                gp_labels_y = self.__gp_predictor_y.train_targets
                self.__gp_predictor_x.set_train_data(torch.cat([gp_data,point],dim=0),torch.cat([gp_labels_x,torch.FloatTensor([wind_force[0]])]),strict=False)
                self.__gp_predictor_y.set_train_data(torch.cat([gp_data,point],dim=0),torch.cat([gp_labels_y,torch.FloatTensor([wind_force[1]])]),strict=False)
            t+=1
        ...
        fig, ax = plt.subplots(2,1)
        fig.set_size_inches(16,9)
        fig.suptitle('Multi-Step Ahead Prediction (x-axis)')
        fig.tight_layout(pad=3.0)
        ax[0].set_xlim([0,T[-1]])
        ax[0].plot(T[max_size:],x_pred,'b-',label="estimated Wind Force")
        ax[0].plot(T,self.__wind_force_x,'--',color='orange',label='Real Wind Force')
        ...
        fig, ax = plt.subplots(2,1)
        fig.set_size_inches(16,9)
        fig.suptitle('Multi-Step Ahead Prediction (y-axis)')
        fig.tight_layout(pad=3.0)
        ax[0].set_xlim([0,T[-1]])
        ax[0].plot(T[max_size:],y_pred,'b-',label="estimated Wind Force")
        ax[0].plot(T,self.__wind_force_y,'--',color='orange',label='Real Wind Force')
