...
opt = json.load(file)
exact_gp_options = {k:True if opt[k]=='True' else False for k in opt.keys()}
file.close()
## MultiOutput ExactGP
file = open('configs/mo_exact_gp.json')
opt = json.load(file)
mo_exact_gp_options = {k:True if opt[k]=='True' else False for k in opt.keys()}
file.close()
## SVGP
file = open('configs/svgp.json')
opt = json.load(file)
svgp_options = {k:True if opt[k]=='True' else False for k in opt.keys()}
file.close()
